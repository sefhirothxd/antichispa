;(function () {
    ...
    //日期选择器，未来的不能改
    var dateInputnow = $("input.js-date-now")
    if (dateInputnow.length) {
        Wind.use('datePicker', function () {
            dateInputnow.datePicker({max: new Date()});
        });
    }
    
    //日期选择器，之前时间不能改
    var dateInputNext = $("input.js-date-next")
    if (dateInputNext.length) {
        Wind.use('datePicker', function () {
        	dateInputNext.datePicker({min: new Date()});
        });
    }
...
function open_iframe_dialog_byWidth(url, title, percentage, options) {
    var params = {
        title : title,
        lock : true,
        opacity : 0,
